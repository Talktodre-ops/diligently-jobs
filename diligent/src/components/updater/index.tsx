...
interface UpdaterProps {
  /** When true, skip the auto-check-on-mount and don't auto-open the popover.
   *  The worst possible time to prompt for an update is mid-interview. */
  capturing?: boolean;
}

export const Updater = ({ capturing = false }: UpdaterProps) => {
  const updaterEnabled = import.meta.env.VITE_ENABLE_UPDATER === "true";
  const manualDownloadUrl =
    import.meta.env.VITE_UPDATE_DOWNLOAD_URL || "https://diligently.app/downloads";
  ...
  // Check for updates on component mount — but defer while the user is
  // mid-interview. We re-check when capturing transitions back to false.
  useEffect(() => {
    if (!updaterEnabled) return;
    if (capturing) return;
    checkForUpdates();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [updaterEnabled, capturing]);
  ...
  if (!updaterEnabled || updateState === "uptodate" || updateState === "error") {
    return null;
  }
