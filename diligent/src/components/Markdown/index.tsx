...
export function Markdown({ children }: MarkdownRendererProps) {
  // Wrapped in an error boundary so a malformed Mermaid block / partial
  // streaming chunk / shiki crash can't unmount the whole React tree.
  // (That was the "app disappears mid-stream but stays in the taskbar"
  // failure mode.)
  return (
    <MarkdownErrorBoundary fallbackText={children}>
      <ReactMarkdown
...
const COMPONENTS = {
  ...
  code: ({ children, className, ...rest }: any) => {
    ...
    if (match && match[1] === "mermaid") {
      // System-design diagrams. The model emits ```mermaid blocks; render as
      // SVG via the Mermaid library instead of as syntax-highlighted code.
      const raw =
        typeof children === "string"
          ? children
          : childrenTakeAllStringContents(children);
