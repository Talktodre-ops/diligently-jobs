...
export function processUserMessageTemplate(
  ...
  // JSON.stringify(template) produces a JSON string where {{TEXT}} sits inside
  // a quoted string value. The replacement must also be JSON-safe, otherwise
  // any newline / quote / tab in userMessage breaks JSON.parse below.
  // JSON.stringify(userMessage) gives `"text"` — slicing off the outer quotes
  // gives the correctly-escaped interior to drop straight into the JSON string.
