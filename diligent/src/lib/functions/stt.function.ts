...
// App-level STT path: routes audio through the Rust transcribe_audio command
// (Deepgram via DEEPGRAM_API_KEY in src-tauri/.env, with a managed-API fallback).
async function fetchManagedSTT(audio: File | Blob): Promise<string> {
  try {
...
export async function fetchSTT(params: STTParams): Promise<string> {
  ...
  try {
    ...
    // Managed-API mode short-circuits straight to the Rust path.
    const useManagedApi = await shouldUseManagedApi();
    if (useManagedApi) {
      return await fetchManagedSTT(audio);
    }

    // App-level fallback:
    // If STT provider is not explicitly selected, use Rust `transcribe_audio`.
    // This lets DEEPGRAM_API_KEY from src-tauri/.env work without Settings setup.
    if (!selectedProvider?.provider) {
      return await fetchManagedSTT(audio);
    }

    // If Deepgram template is selected but no API key is configured in Settings,
    // use app-level Deepgram from Rust/env instead of sending invalid credentials.
    if (provider?.id === "deepgram-stt") {
      const deepgramKey = selectedProvider.variables?.API_KEY?.trim();
      if (!deepgramKey) {
        return await fetchManagedSTT(audio);
      }
    }
