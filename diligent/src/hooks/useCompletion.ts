...
import { fetchAIResponse, safeLocalStorage, compressScreenshot } from "@/lib";
...
export const useCompletion = () => {
  ...
  const resolveProvider = useCallback(() => {
    const selectedProviderId = selectedAIProvider.provider;
    if (selectedProviderId) {
      return allAiProviders.find((p) => p.id === selectedProviderId);
    }
  ...
  const submit = useCallback(
    async (speechText?: string) => {
      ...
      try {
        ...
        // Vision routing: when an image is attached, force this turn to
        // Claude regardless of the user's text-default provider. DeepSeek
        // (our Ask-Me-Anything default) has no vision and rejects
        // image_url content with `unknown variant image_url, expected
        // text`. Claude's reply still joins the same chat history, so
        // follow-up text-only turns route back to DeepSeek with the prior
        // turn as context. If the user *already* picked Claude, keep their
        // settings-set variables (custom api_key etc.); otherwise blank
        // variables out so the env autofill in api.rs picks ANTHROPIC_*.
        const hasImage = imagesBase64.length > 0;
        const userAlreadyChoseClaude = selectedAIProvider.provider === "claude";
        const baseProvider = resolveProvider();
        const provider = hasImage && !userAlreadyChoseClaude
          ? (allAiProviders.find((p) => p.id === "claude") ?? baseProvider)
          : baseProvider;
        const effectiveSelected = hasImage && !userAlreadyChoseClaude
          ? { provider: "claude", variables: {} as Record<string, string> }
          : selectedAIProvider;
        ...
        // Pre-validate required variables for providers without env-backed keys.
        // Claude IS env-backed (ANTHROPIC_API_KEY autofill in ai-response.function.ts)
        // but the original list missed it — fixed here so a Claude-only chat
        // doesn't require manually pasting the key into Settings variables.
        if (provider && !useManagedApi) {
          const providerId = provider.id ?? "";
          const hasEnvBacking =
            providerId === "openai" || providerId === "deepseek" || providerId === "claude";
          if (!hasEnvBacking) {
            ...
            const requiredVarRe = /\{\{([A-Z_]+)\}\}/g;
            const missing: string[] = [];
            let m: RegExpExecArray | null;
            while ((m = requiredVarRe.exec(provider.curl)) !== null) {
              const key = m[1];
              if (!systemKeys.has(key)) {
                const varKey = key.toLowerCase();
                if (!vars[varKey] || vars[varKey].trim() === "") {
                  missing.push(key);
                }
              }
            }
            if (missing.length > 0) {
              setState((prev) => ({
                ...prev,
                error: `Missing required settings: ${missing.join(", ")}. Open Settings to configure.`,
              }));
              return;
            }
          }
        ...
        try {
          // Use the fetchAIResponse function. effectiveSelected handles
          // the image→Claude override above.
          for await (const chunk of fetchAIResponse({
            provider: useManagedApi ? undefined : provider,
            selectedProvider:
              useManagedApi || !provider?.id
                ? effectiveSelected
                : { ...effectiveSelected, provider: provider.id },
  ...
  useEffect(() => {
    const handleConversationSelected = (event: Event) => {
      const conversation = (event as CustomEvent<ChatConversation>).detail;
    ...
    try {
      if (prompt) {
        ...
        try {
          ...
          // Vision routing — same logic as submit(): screenshot turns
          // always go to Claude because DeepSeek (our text default) has
          // no vision. The auto-prompt + screenshot becomes a one-shot
          // Claude vision call; the answer enters history so subsequent
          // text-only follow-ups can route back to DeepSeek.
          const userAlreadyChoseClaude = selectedAIProvider.provider === "claude";
          const baseProvider = resolveProvider();
          const provider = !userAlreadyChoseClaude
            ? (allAiProviders.find((p) => p.id === "claude") ?? baseProvider)
            : baseProvider;
          const effectiveSelected = !userAlreadyChoseClaude
            ? { provider: "claude", variables: {} as Record<string, string> }
            : selectedAIProvider;
  ...
  const captureScreenshot = async () => {
    ...
    try {
      const raw = await invoke<string>("capture_to_base64");
      // Compress from full-res PNG to 1280px-wide JPEG — ~10-20x smaller payload
      const base64 = await compressScreenshot(raw);

      if (screenshotConfiguration.mode === "auto") {
        handleScreenshotSubmit(base64, screenshotConfiguration.autoPrompt);
      } else if (screenshotConfiguration.mode === "manual") {
        handleScreenshotSubmit(base64);
      }
    } catch (error) {
