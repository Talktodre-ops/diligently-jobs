...
pub async fn secure_storage_save(app: AppHandle, items: Vec<StorageItem>) -> Result<(), String> {
    ...
    for item in items {
        match item.key.as_str() {
            "managed_license_key" => storage.license_key = Some(item.value),
            "managed_instance_id" => storage.instance_id = Some(item.value),
            "selected_managed_model" => storage.selected_managed_model = Some(item.value),
...
pub async fn secure_storage_get(app: AppHandle) -> Result<StorageResult, String> {
    ...
    Ok(StorageResult {
        ...
        selected_managed_model: storage.selected_managed_model,
    })
}
...
pub async fn secure_storage_remove(app: AppHandle, keys: Vec<String>) -> Result<(), String> {
    ...
    for key in keys {
        match key.as_str() {
            "managed_license_key" => storage.license_key = None,
            "managed_instance_id" => storage.instance_id = None,
            "selected_managed_model" => storage.selected_managed_model = None,
